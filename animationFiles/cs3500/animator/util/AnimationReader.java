...
public class AnimationReader {
   ...
   * @return the document produced by the builder
   */
  ...
  private static <Doc> void readKeyframe(Scanner s, AnimationBuilder<Doc> builder) {
    String[] fieldNames = new String[]{
      "time",
    ...
    if (s.hasNext()) {
      ...
      throw new IllegalStateException("Keyframe: Expected a shape name, but no more input available");
    }
    for (int i = 0; i < 8; i++) {
      vals[i] = getInt(s, "Keyframe", fieldNames[i]);
    }
    builder.addKeyframe(name, vals[0], vals[1], vals[2], vals[3],
